...
public class DatumErweiterung {
	public static void main(String [] args) {
		...
		if ((years) && (months) && (days) && (hours) && (minminutes) && (plusminutes)) { // Prüfen ob die eingegebenen Werte im Wertebereich sind
			if (((februar) && (t > 28)) || ((kurzerMonat) && (t > 30)) || (langerMonat) && (t > 31)) { // Prüfen ob die eingegeben Tage zu den entsprechenden Monaten passt
				...
				if ((m1 + 1) > 59) {
					m1 = 0;			
			     	...
			     	if (s > 23) { // Prüfen ob die Stunden größer 23 sind, wenn ja dann s auf 0 setzen und den Tag + 1 rechnen
						 s = 0;
						 t = t + 1;
						 if ((februar) && (t > 28)) { // Prüfen ob der Monat Februar ist und mehr als 28 Tage hat, dann Tag + 1 und Monat + 1 rechnen
							 t = 1;
							 m = m + 1;
						 } else { 
							 if ((kurzerMonat) && (t > 30)) { // Prüfen ob der Monat zu den Monaten mit 30 Tagen zählt und ob dieser mehr als 30 Tage hat 
								 t = 1;		
								 m = m + 1;
								} else {
									if ((langerMonat) && (t > 31)) {  // Prüfen ob der Monat zu den Monaten mit 31 Tagen zählt und ob dieser mehr als 31 Tage hat 
										t = 1;
										m = m + 1;
			...
			System.out.println("Einer der Werte ist außerhalb des Wertebereichs"
					...
					+ "\nJahr: 1600 <= " + j + " <= 3000"
					+ "\nMonat: 1 <= " + m + " <= 12"
					+ "\nTag: 1 <= " + t + " <= 31 - Eingrenzungen sind hier zudem noch die unterschiedlichen Längen der einzelnen Monate"
					+ "\nStunden: 0 <= " + s + " <= 23"
					+ "\nMinuten: 0 <= " + m1 + " <= 59"
