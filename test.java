...
public class test {
	public static void main(String [] args) {
		...
		if ((years) && (months) && (days) && (hours) && (minminutes) && (plusminutes)) { // Prüfen ob das Jahr im Wertebereich liegt
			if ((februar) || ((kurzerMonat) && (t > 30)) || ((langerMonat) && (t > 31))) { // Prüfen ob angegebene Tage zu den richtigen Monaten passt
			     	...
			     	if (s > 23) { // Prüfen ob die Stunden größer 23 sind, wenn ja dann s auf 0 setzen und den Tag + 1 rechnen
						 ...
						 if ((februar) && (t > 28)) { // Prüfen ob der Monat Februar ist und mehr als 28 Tage hat, dann Tag + 1 und Monat + 1 rechnen
							 ...
							 m = m + 1;
									...
									m = m + 1;
								} else { if ((langerMonat) && (t > 31)) {  // Prüfen ob der Monat zu den Monaten mit 31 Tagen zählt und ob dieser mehr als 31 Tage hat 
									t = 1;
									m = m + 1;
			...
			System.out.println("Einer der Werte ist außerhalb des Wertebereichs"
					...
					+ "\nJahr: >= 1600 und <= 3000"
					+ "\nMonat: >= 1 und <= 12"
					+ "\nTag: >= 1 und <= 31 - Ausnahmen sind hier zudem noch die unterschiedlichen Längen der einzelnen Monate"
					+ "\nStunden: >= 0 und <= 23"
					+ "\nMinuten: >= 0 und <= 59"
					+ "\nMinuten plus: >= 0 und <= 40000"
